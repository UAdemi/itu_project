...
```{r Sentiment Analysis with Tidy - Biden Corpus}
# Tidy up the DTM
...
# Create biden_count_afinn by forwarding ag_afinn_words to count(), passing in value, index.
...
biden_polarity_plot_afinn <- biden_afinn_words %>%
                    select(term, count, index)

head(biden_polarity_plot,10)
...
```{r Sentiment Analysis with Tidy - Biden Corpus - 2 }
...
# Plot polarity vs. index
ggplot(ag_polarity, aes(x = index, y = polarity)) + 
  # Add a smooth trend curve
  geom_smooth()
